...
NVENCSTATUS NVEncoderAPI::NvEncOpenEncodeSession(void* device, uint32_t deviceType)
{
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodeGUIDCount(uint32_t* encodeGUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodeProfileGUIDCount(GUID encodeGUID, uint32_t* encodeProfileGUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodeProfileGUIDs(GUID encodeGUID, GUID* profileGUIDs, uint32_t guidArraySize, uint32_t* GUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodeGUIDs(GUID* GUIDs, uint32_t guidArraySize, uint32_t* GUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetInputFormatCount(GUID encodeGUID, uint32_t* inputFmtCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetInputFormats(GUID encodeGUID, NV_ENC_BUFFER_FORMAT* inputFmts, uint32_t inputFmtArraySize, uint32_t* inputFmtCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodeCaps(GUID encodeGUID, NV_ENC_CAPS_PARAM* capsParam, int* capsVal)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodePresetCount(GUID encodeGUID, uint32_t* encodePresetGUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodePresetGUIDs(GUID encodeGUID, GUID* presetGUIDs, uint32_t guidArraySize, uint32_t* encodePresetGUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodePresetConfig(GUID encodeGUID, GUID  presetGUID, NV_ENC_PRESET_CONFIG* presetConfig)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncCreateInputBuffer(uint32_t width, uint32_t height, void** inputBuffer, NV_ENC_BUFFER_FORMAT inputFormat)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncDestroyInputBuffer(NV_ENC_INPUT_PTR inputBuffer)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncCreateMVBuffer(uint32_t size, void** bitstreamBuffer)
{
    NVENCSTATUS status;
...
NVENCSTATUS NVEncoderAPI::NvEncDestroyMVBuffer(NV_ENC_OUTPUT_PTR bitstreamBuffer)
{
    NVENCSTATUS status;
    NV_ENC_CREATE_MV_BUFFER stAllocMVBuffer;
    memset(&stAllocMVBuffer, 0, sizeof(stAllocMVBuffer));
...
NVENCSTATUS NVEncoderAPI::NvRunMotionEstimationOnly(MotionEstimationBuffer *pMEBuffer, MEOnlyConfig *pMEOnly)
{
    NVENCSTATUS nvStatus;
    NV_ENC_MEONLY_PARAMS stMEOnlyParams;
    SET_VER(stMEOnlyParams,NV_ENC_MEONLY_PARAMS);
    stMEOnlyParams.referenceFrame = pMEBuffer->stInputBfr[0].bDeviceSurface ?
    								pMEBuffer->stInputBfr[0].hDeviceInputSurface : pMEBuffer->stInputBfr[0].hHostInputSurface;
    stMEOnlyParams.inputBuffer = pMEBuffer->stInputBfr[0].bDeviceSurface ?
								 pMEBuffer->stInputBfr[0].hDeviceInputSurface : pMEBuffer->stInputBfr[0].hHostInputSurface;
...
NVENCSTATUS NVEncoderAPI::NvEncCreateBitstreamBuffer(uint32_t size, void** bitstreamBuffer)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncDestroyBitstreamBuffer(NV_ENC_OUTPUT_PTR bitstreamBuffer)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncLockBitstream(NV_ENC_LOCK_BITSTREAM* lockBitstreamBufferParams)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncUnlockBitstream(NV_ENC_OUTPUT_PTR bitstreamBuffer)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncLockInputBuffer(void* inputBuffer, void** bufferDataPtr, uint32_t* pitch)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncUnlockInputBuffer(NV_ENC_INPUT_PTR inputBuffer)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetEncodeStats(NV_ENC_STAT* encodeStats)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncGetSequenceParams(NV_ENC_SEQUENCE_PARAM_PAYLOAD* sequenceParamPayload)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncRegisterAsyncEvent(void** completionEvent)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncUnregisterAsyncEvent(void* completionEvent)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
    NV_ENC_EVENT_PARAMS eventParams;
...
NVENCSTATUS NVEncoderAPI::NvEncMapInputResource(void* registeredResource, void** mappedResource)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncUnmapInputResource(NV_ENC_INPUT_PTR mappedInputBuffer)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncDestroyEncoder()
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncInvalidateRefFrames(const NvEncPictureCommand *pEncPicCommand)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncOpenEncodeSessionEx(void* device, NV_ENC_DEVICE_TYPE deviceType)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncRegisterResource(NV_ENC_INPUT_RESOURCE_TYPE resourceType, void* resourceToRegister,
...
NVENCSTATUS NVEncoderAPI::NvEncUnregisterResource(NV_ENC_REGISTERED_PTR registeredRes)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncReconfigureEncoder(const NvEncPictureCommand *pEncPicCommand)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::ValidateEncodeGUID (GUID inputCodecGuid)
{
...
NVENCSTATUS NVEncoderAPI::ValidatePresetGUID(GUID inputPresetGuid, GUID inputCodecGuid)
{
...
NVENCSTATUS NVEncoderAPI::CreateEncoder(EncodeConfig *pEncCfg)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
    ...
    m_stCreateEncodeParams.frameRateNum = pEncCfg->frame_rate_num;
    m_stCreateEncodeParams.frameRateDen = pEncCfg->frame_rate_den;
   ...
   // m_stEncodeConfig.frameIntervalP = pEncCfg->numB + 1;
    ...
    if(pEncCfg->codec == NV_ENC_HEVC){
		//m_stEncodeConfig.encodeCodecConfig.hevcConfig.level = 4;
		m_stEncodeConfig.encodeCodecConfig.hevcConfig.outputAUD = 0;
		m_stEncodeConfig.encodeCodecConfig.hevcConfig.maxCUSize = NV_ENC_HEVC_CUSIZE_AUTOSELECT;
		m_stEncodeConfig.encodeCodecConfig.hevcConfig.maxNumRefFramesInDPB = pEncCfg->refnum;
		m_stEncodeConfig.encodeCodecConfig.hevcConfig.repeatSPSPPS = 1;
		//m_stEncodeConfig.encodeCodecConfig.hevcConfig.pixelBitDepthMinus8 = 2;
    }else{
    	m_stEncodeConfig.encodeCodecConfig.h264Config.outputAUD = 0;
    	m_stEncodeConfig.encodeCodecConfig.h264Config.maxNumRefFrames = pEncCfg->refnum;
...
GUID NVEncoderAPI::GetPresetGUID(char* encoderPreset, int codec)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::ProcessOutput(const EncodeBuffer *pEncodeBuffer,NV_ENC_LOCK_BITSTREAM & lockBitstreamData){
...
NVENCSTATUS NVEncoderAPI::ProcessOutput(const EncodeBuffer *pEncodeBuffer, unsigned char ** buffer, unsigned int& charcount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
    ...
    if (nvStatus == NV_ENC_SUCCESS)
    {
		*buffer =(unsigned char *) lockBitstreamData.bitstreamBufferPtr;
		charcount = lockBitstreamData.bitstreamSizeInBytes;
        nvStatus = m_pEncodeAPI->nvEncUnlockBitstream(m_hEncoder, pEncodeBuffer->stOutputBfr.hBitstreamBuffer);
    }
    else
    {
...
NVENCSTATUS NVEncoderAPI::ProcessMVOutput(const MotionEstimationBuffer *pMEBuffer)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::Initialize(void* device, NV_ENC_DEVICE_TYPE deviceType)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS NVEncoderAPI::NvEncEncodeFrame(EncodeBuffer *pEncodeBuffer, NvEncPictureCommand *encPicCommand,
                                           ...
                                           NVENC_EXTERNAL_ME_HINT *meExternalHints, NVENC_EXTERNAL_ME_HINT_COUNTS_PER_BLOCKTYPE meHintCountsPerBlock[])
{
   ...
   // encPicParams.inputBuffer = pEncodeBuffer->stInputBfr.hInputSurface;
    ...
    if(pEncodeBuffer->stInputBfr.bDeviceSurface){
    	encPicParams.inputBuffer = pEncodeBuffer->stInputBfr.hDeviceInputSurface;
    }else
    	encPicParams.inputBuffer = pEncodeBuffer->stInputBfr.hHostInputSurface;
...
NVENCSTATUS NVEncoderAPI::NvEncFlushEncoderQueue(void *hEOSEvent)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
    NV_ENC_PIC_PARAMS encPicParams;
    memset(&encPicParams, 0, sizeof(encPicParams));
